...
def process_ffmpeg(path, clip_name, start_number, end_number):
    
    files = [f for f in os.listdir(path) if f.endswith('.jpg')]
    files = [f for f in files if f.split('.')[0].isdigit() and start_number <= int(f.split('.')[0]) <= end_number]
    ...
    temp_dir = os.path.join(path, 'temp_ffmpeg')
...
def split_image(img, overlap_height):
    height, width = img.shape[:2] 
    split_height = height // 2

    top1 = 0
    bottom1 = split_height + overlap_height
    top2 = split_height - overlap_height
    bottom2 = height

    img1 = img[top1:bottom1, 0:width]
    img2 = img[top2:bottom2, 0:width]

    return img1, img2

def merge_images(img1, img2, overlap_height):
    height1, width1 = img1.shape[:2]
    height2, width2 = img2.shape[:2]

    new_height = height1 + height2 - overlap_height
    new_img = np.zeros((new_height, width1, 3), dtype=img1.dtype)

    new_img[0:height1, 0:width1] = img1

    new_img[height1 - overlap_height:new_height, 0:width2] = img2
...
def process_frame(frame, mog_subtractor, frame_count):
    ...
    output_path = f"cache/convert/{frame_count}.jpg"
    cv2.imwrite(output_path, convert_image)
    temp1,temp2 = split_image(convert_image, 20)
    cv2.imwrite(f"cache/convert_up/{frame_count}.jpg", temp1)
    cv2.imwrite(f"cache/convert_down/{frame_count}.jpg", temp2)
...
def save_clips(capture, clips, output_path):
    ...
    for i in range(0, len(clips), 2):
        ...
        process_ffmpeg("cache/convert",f"{output_path}/clip_convert_{i // 2 + 1}_{start_frame}-{end_frame}",start_frame,end_frame)
        process_ffmpeg("cache/convert_up",f"{output_path}/clip_convert_up_{i // 2 + 1}_{start_frame}-{end_frame}",start_frame,end_frame)
        process_ffmpeg("cache/convert_down",f"{output_path}/clip_convert_down_{i // 2 + 1}_{start_frame}-{end_frame}",start_frame,end_frame)
...
if __name__ == "__main__":
    ...
    os.makedirs("cache/convert", exist_ok=True)
    os.makedirs("cache/convert_up", exist_ok=True)
    os.makedirs("cache/convert_down", exist_ok=True)
